...
export async function generateCertificatePDF(data: CertificateData): Promise<Uint8Array> {
  ...
  doc.text('REPUBLIC OF THE PHILIPPINES', centerX, 25, { align: 'center' });
  doc.text('PROVINCE OF ILOCOS NORTE', centerX, 30, { align: 'center' });
  doc.text('MUNICIPALITY OF ASUNCION', centerX, 35, { align: 'center' });

  doc.setFontSize(9);
  doc.setFont('helvetica', 'italic');
  doc.text('Public Employment Service Office (PESO)', centerX, 40, { align: 'center' });
  ...
  doc.line(60, 66, pageWidth - 60, 66);
  ...
  doc.setFontSize(20);
  doc.setFont('helvetica', 'bold');
  doc.text(data.trainee.full_name.toUpperCase(), centerX, 92, { align: 'center' });

  // Underline name
  const nameWidth = doc.getTextWidth(data.trainee.full_name.toUpperCase());
  doc.setLineWidth(0.5);
  doc.setDrawColor(0, 0, 0);
  doc.line(centerX - nameWidth / 2, 94, centerX + nameWidth / 2, 94);

  // Program completion text
  doc.setFontSize(12);
  doc.setFont('helvetica', 'normal');
  doc.text('has successfully completed the training program in', centerX, 105, { align: 'center' });

  // Program title (highlighted)
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text(data.program.title, centerX, 115, { align: 'center' });
 ...
 */
export async function generateCertificatePreview(data: CertificateData): Promise<Blob> {
